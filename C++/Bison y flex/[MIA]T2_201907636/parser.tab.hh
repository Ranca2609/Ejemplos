...
switch (yytype)
    {
      ...
      case 3: // "id"
      case 4: // "reservada_rep"
      case 5: // "reservada_path"
      case 6: // "reservada_exec"
      case 7: // "reservada_mkdisk"
      case 8: // "ruta"
      case 9: // "igual"
      case 13: // INSTRUCCIONES
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      symbol_type (int tok, std::string v, location_type l)
        : super_type(token_type (tok), std::move (v), std::move (l))
      {
        YY_ASSERT (tok == token::TOK_IDENTIFICADOR || tok == token::TOK_REP || tok == token::TOK_PATH || tok == token::TOK_EXEC || tok == token::TOK_MKDISK || tok == token::TOK_RUTA || tok == token::TOK_IGUAL);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_REP (std::string v, location_type l)
      {
        return symbol_type (token::TOK_REP, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_REP (const std::string& v, const location_type& l)
      {
      ...
      static
      symbol_type
      make_EXEC (std::string v, location_type l)
      {
        return symbol_type (token::TOK_EXEC, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      make_EXEC (const std::string& v, const location_type& l)
      {
      ...
      {
        return symbol_type (token::TOK_IGUAL, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      ...
      {
        return symbol_type (token::TOK_IGUAL, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
  ...
  calculadora_parser::yytranslate_ (int t)
  {
...
#if 201103L <= YY_CPLUSPLUS
  template <typename Base>
  calculadora_parser::basic_symbol<Base>::basic_symbol (basic_symbol&& that)
    : Base (std::move (that))
    , value ()
...
#if 201103L <= YY_CPLUSPLUS
  inline
  calculadora_parser::by_type::by_type (by_type&& that)
    : type (that.type)
  {
    that.clear ();
  }
  ...
  inline
  calculadora_parser::by_type::by_type (const by_type& that)
    : type (that.type)
  {}
  ...
  inline
  int
  calculadora_parser::by_type::type_get () const YY_NOEXCEPT
  {
    return type;
  }
